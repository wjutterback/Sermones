...
function printSleepLessTwo(canvasLessTwo) {
  ...
  canvasLessTwo
    .getContext('2d')
    .drawImage(video, 0, 0, canvasLessTwo.width, canvasLessTwo.height);
  imgDataLessTwo = canvasLessTwo
    .getContext('2d')
    .getImageData(0, 0, canvasLessTwo.width, canvasLessTwo.height);
}
function printSleepLessOne(canvasLessOne) {
  ...
  canvasLessOne
    .getContext('2d')
    .drawImage(video, 0, 0, canvasLessOne.width, canvasLessOne.height);
  imgDataLessOne = canvasLessOne
    .getContext('2d')
    .getImageData(0, 0, canvasLessOne.width, canvasLessOne.height);
}
...
function printSleepII(arguments) {
  ...
  try {
    ...
    for (var i = 0; i <= imgDataLessOne.data.length / 4 - 1; i++) {
      ...
      pixelShow = Number(
        !(
          Math.abs(imgData.data[4 * i] - imgDataLessOne.data[4 * i]) <
            RGBThresh &&
          Math.abs(imgData.data[4 * i + 1] - imgDataLessOne.data[4 * i + 1]) <
            RGBThresh &&
          Math.abs(imgData.data[4 * i + 2] - imgDataLessOne.data[4 * i + 2]) <
            RGBThresh
        )
      );
      // debugger;
    ...
    var idata = new ImageData(
      new Uint8ClampedArray(rgb_diff),
      canvas.width,
      canvas.height
    );
